...
t_data <- as.data.table(t_anomaly.array) %>% 
  ...
  mutate(longtitude = lon[longtitude],
         latitude = lat[latitude],
         time = t[time] + as.Date("1800-01-01"),
         year = year(time)) %>%
  group_by(year, longtitude, latitude) %>% 
...
t_data %>%
  mutate(t_tiff = case_when(
    t_diff < -4 ~ -4,
    t_diff > 4 ~ 4,
    TRUE ~ t_diff
  )) %>% 
  # filter(year == 2000) %>% 
  ggplot(aes(x = longtitude, y = latitude, fill = t_diff)) +
  geom_raster() +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Global annual temperature anomalies(1950-2022)"
  ) +
  theme(
