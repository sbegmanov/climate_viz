...
this_month <- month(today() - 1, label = TRUE, abbr = FALSE)
this_day <- ordinal(day(today() - 1))
...
local_weather %>% 
  ...
  filter(year != 1951 & !(month == 2 & day == 29)) %>% 
  arrange(date) %>% 
  group_by(year) %>% 
  mutate(cum_prcp = cumsum(prcp)) %>% 
  ungroup() %>% 
  mutate(new_date = ymd(glue("2022-{month}-{day}"))) %>% # pseudo date
  ...
  labs(x = NULL,
       ...
       title = glue("Through {this_month} {this_day}, 
                    the cumulative precipitation near Ann Arbor, 
                    MI is <span style = 'color: dodgerblue'>above average</span> for {this_year}")) +
  theme(
