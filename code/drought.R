...
drought_data <- local_weather %>% 
  ...
  mutate(threshold = quantile(window_prcp, probs = 0.1)) %>% 
  ungroup() #%>% 
...
drought_data %>% 
  mutate(fake_date = ymd(glue("2020-{end_month}-{end_day}"))) %>% 
  select(-start, -end) %>% 
  mutate(is_drougt_year = end_year == 2012,
         end_year = fct_reorder(factor(end_year), is_drougt_year)) %>% 
  ggplot(aes(x = fake_date, y = window_prcp, group = end_year, color = is_drougt_year)) +
  ...
  labs(
    ...
    title = "The summer of <span style='color:dodgerblue'>2012</span>
    had less precipitation than <span style='color:red'>95% 
    of previous years dating back to 1892</span>"
  ) +
  theme(
