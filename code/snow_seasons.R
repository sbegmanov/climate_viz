...
snow_data <- local_weather %>% 
  ...
  filter(snow_year != 1981 & snow_year != 2023)
...
# fulfill missed dates
dummy_df <- crossing(snow_year = 1892:2022,
                     month = 1:12) %>% 
  mutate(dummy = 0)
...
snow_data %>% 
  ...
  labs(x = NULL,
       ...
       title = glue("The <span style = 'color:dodgerblue'>snow year 2022 </span> had a total of {total_snow} cm of snow")) +
  theme(
